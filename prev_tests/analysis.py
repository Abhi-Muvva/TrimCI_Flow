"""
analysis.py
===========
Post-processing and analysis for TrimCI-Flow results.

Functions
---------
- determinant_summary    : print/return table of per-fragment det counts vs reference
- plot_det_comparison    : bar chart of fragment dets vs brute-force
- orbital_mi_analysis    : compute and visualize orbital mutual information
- plot_mi_heatmap        : heatmap of the MI matrix
"""

from __future__ import annotations

from typing import Optional
import numpy as np


BRUTE_FORCE_DETS = 10095    # TrimCI on full Fe4S4, 10K-determinant run
REFERENCE_ENERGY = -327.1920  # Ha, from Dr. Otten's group


def determinant_summary(result) -> dict:
    """
    Print and return a summary table comparing fragment determinant usage
    to the brute-force TrimCI reference.

    Parameters
    ----------
    result : FragmentedRunResult from trimci_flow.py

    Returns
    -------
    summary : dict with keys:
        'fragment_dets'    : list of per-fragment det counts
        'total_dets'       : sum
        'brute_force_dets' : BRUTE_FORCE_DETS (10095)
        'ratio'            : total_dets / brute_force_dets
        'savings_pct'      : (1 - ratio) * 100  [positive = fewer dets]
    """
    lines = []
    lines.append("=" * 60)
    lines.append("TrimCI-Flow Path C — Determinant Summary")
    lines.append("=" * 60)

    for i, (orbs, n_dets, energy) in enumerate(
        zip(result.fragment_orbs, result.fragment_n_dets, result.fragment_energies)
    ):
        lines.append(
            f"  Fragment {i}: orbs [{orbs[0]:2d}..{orbs[-1]:2d}]"
            f"    n_dets={n_dets:6d}"
            f"    energy={energy:12.4f} Ha"
        )

    lines.append("-" * 60)
    total = result.total_dets
    ref   = result.brute_force_dets
    ratio = total / ref if ref > 0 else float("nan")
    savings = (1.0 - ratio) * 100.0

    lines.append(f"  Total dets     : {total:>8d}")
    lines.append(f"  Brute-force ref: {ref:>8d}  (E = {REFERENCE_ENERGY:.4f} Ha)")
    lines.append(f"  Ratio          : {ratio:>8.3f}x brute-force")
    lines.append(f"  Savings        : {savings:>+7.1f}%  (negative = MORE dets than brute-force)")
    lines.append("=" * 60)

    report = "\n".join(lines)
    print(report)

    return {
        "fragment_dets":    result.fragment_n_dets,
        "total_dets":       total,
        "brute_force_dets": ref,
        "ratio":            ratio,
        "savings_pct":      savings,
    }


def iteration_summary(result) -> list:
    """
    Print an iteration table and return the iteration history from a Phase B run.

    Parameters
    ----------
    result : FragmentedRunResult from run_selfconsistent_fragments

    Returns
    -------
    history : list of dicts, one per iteration, each with keys:
        'iteration', 'energies', 'n_dets', 'delta_E', 'delta_rdm'
    """
    history = list(getattr(result, 'iteration_history', []))
    if not history:
        print("No iteration history recorded.")
        return history

    print("=" * 78)
    print("Phase B — Iteration Summary")
    print("=" * 78)
    print(f"  {'Iter':>4}  {'max|ΔE| (Ha)':>14}  {'max|Δγ|':>12}  "
          f"{'total_dets':>10}  energies")
    print("-" * 78)
    for entry in history:
        it      = entry['iteration']
        dE      = entry['delta_E']
        drdm    = entry['delta_rdm']
        ndets   = sum(entry['n_dets'])
        energies_str = "[" + ", ".join(f"{e:.4f}" for e in entry['energies']) + "]"
        dE_str   = f"{dE:.3e}" if dE != float('inf') else "     inf"
        drdm_str = f"{drdm:.3e}" if drdm != float('inf') else "     inf"
        print(f"  {it:>4}  {dE_str:>14}  {drdm_str:>12}  {ndets:>10}  {energies_str}")
    print("=" * 78)
    return history


def convergence_summary(result) -> dict:
    """
    Print a convergence verdict and return a summary dict from a Phase B run.

    Parameters
    ----------
    result : FragmentedRunResult from run_selfconsistent_fragments

    Returns
    -------
    summary : dict with keys:
        'converged'             : bool
        'iterations'            : int, number of SC cycles performed
        'convergence_delta'     : float, final max|ΔE| across fragments
        'convergence_delta_rdm' : float, final max|Δγ| across orbitals
        'verdict'               : str, human-readable status
    """
    converged   = getattr(result, 'converged', False)
    iterations  = getattr(result, 'iterations', 1)
    delta_E     = getattr(result, 'convergence_delta', float('inf'))
    delta_rdm   = getattr(result, 'convergence_delta_rdm', float('inf'))

    history = getattr(result, 'iteration_history', [])
    if not history:
        verdict = "No iterations recorded"
    elif converged:
        verdict = "SCF converged"
    else:
        verdict = "Hit max iterations without convergence"

    print("=" * 60)
    print("Phase B — Convergence Summary")
    print("=" * 60)
    print(f"  converged  : {converged}")
    print(f"  iterations : {iterations}")
    print(f"  final max|ΔE|  : {delta_E:.3e} Ha")
    print(f"  final max|Δγ|  : {delta_rdm:.3e}")
    print(f"  Verdict    : {verdict}")
    print("=" * 60)

    return {
        'converged':             converged,
        'iterations':            iterations,
        'convergence_delta':     delta_E,
        'convergence_delta_rdm': delta_rdm,
        'verdict':               verdict,
    }


def plot_det_comparison(result, save_path: Optional[str] = None):
    """
    Bar chart: per-fragment determinant count vs brute-force reference line.

    Parameters
    ----------
    result    : FragmentedRunResult
    save_path : if provided, save figure to this path; else plt.show()
    """
    raise NotImplementedError


def orbital_mi_analysis(
    dets_npz_path: str,
    n_orb: int = 36,
    n_alpha: int = 27,
    n_beta: int = 27,
) -> np.ndarray:
    """
    Compute orbital mutual information from the TrimCI reference wavefunction.

    Uses trimci.compute_orbital_mutual_information on the existing dets.npz
    (10K-determinant Fe4S4 wavefunction).

    Parameters
    ----------
    dets_npz_path : path to dets.npz (Fe4S4 reference)
    n_orb         : number of orbitals (36)
    n_alpha       : alpha electrons (27)
    n_beta        : beta electrons (27)

    Returns
    -------
    mi_matrix : (n_orb, n_orb) symmetric matrix of orbital mutual information
    """
    raise NotImplementedError


def plot_mi_heatmap(
    mi_matrix: np.ndarray,
    fragment_boundaries: Optional[list[list[int]]] = None,
    save_path: Optional[str] = None,
):
    """
    Heatmap of the orbital mutual information matrix.

    Optionally overlays fragment boundaries as colored rectangles.

    Parameters
    ----------
    mi_matrix           : (n_orb, n_orb) MI matrix from orbital_mi_analysis
    fragment_boundaries : list of orbital index lists (from fragment.py)
    save_path           : if provided, save figure; else plt.show()
    """
    raise NotImplementedError
