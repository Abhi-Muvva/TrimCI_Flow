"""
Run Phase B v4: Anderson-accelerated SCF with n_gamma_avg=5.
...
from TrimCI_Flow.trimci_flow_v4 import run_selfconsistent_fragments_v4
...
OUTPUT_DIR = 'outs_v4_anderson_avg5_200dets'
os.makedirs(OUTPUT_DIR, exist_ok=True)  # must exist before any tee redirect
...
print(f'Starting v4 Anderson run at {datetime.datetime.now().isoformat()}')
result = run_selfconsistent_fragments_v4(
    FCIDUMP,
    window_size=15,
    stride=10,
    max_iterations=60,
...
# --- Iteration table ---
header = f"{'Iter':>5} | {'max|dE| Ha':>12} | {'max|dgamma|':>12} | {'rdm_std10':>12} | {'ndets':>8} | {'mix':>4} | energies"
rows = [header, '-' * len(header)]
for entry in result.iteration_history:
    a_flag = 'A' if entry.get('anderson_used') else 'L'
    rows.append(
        f"  {entry['iteration']:3d} | {entry['delta_E']:12.3e} | "
        f"{entry['delta_rdm']:12.3e} | {entry['rdm_rolling_std']:12.3e} | "
        f"{sum(entry['n_dets']):8d} | {a_flag:>4} | "
...
# --- Metadata ---
metadata = {
    'run_type':               'Phase_B_v4_anderson_avg5',
...
# Late-window stats (iters 40..60 if available, else last 10)
...
if late:
    ...
    n_and  = sum(1 for h in late if h.get('anderson_used'))
    metadata['late_stats_40_60'] = {
        ...
        'anderson_used_count': n_and,
        'total_iters':      len(late),
    }
if very_late:
